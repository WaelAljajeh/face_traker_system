...
class QualityFilter:
    ...
    def _apply_config(self, config: dict):
        """Apply config values (can be called at runtime to update mode)."""
    ...
    def apply_mode(self, mode_config: dict):
        """Switch quality parameters for a mode profile at runtime."""
        with self._lock:
            self._apply_config(mode_config)
        print(f"[QUALITY] Mode applied: blur={self.blur_threshold}, min_face={self.min_face_size}")
    ...
    def assess(self, frame: np.ndarray, detection: dict, 
        ...
        if self.blur_detection:
            ...
            if blur_score < blur_threshold:
                reasons['blur'] = f"{blur_score:.1f} < {blur_threshold}"
                quality_score *= 0.3
